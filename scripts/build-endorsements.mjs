...
async function main() {
  ...
  const files = fs.readdirSync(endorsementsDir).filter((file) => file.endsWith(".json"));
  const rawItems = files.map((file) => safeReadJson(path.join(endorsementsDir, file))).filter(Boolean);
